...
def get_youtube_caption(ch, term_start, term_end, next):
    ...
    request = urllib.urlopen(
        "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCaption=closedCaption&channelId="
        + ch
        + term_start
        + term_end
        + "&maxResults=50&key="
    ...
    for video_id in video_id_list:
        ...
        for item in data["items"]:
            ...
            trackKind = item["snippet"][
                "trackKind"
            ]  # "standard" -> manual, "ASR" -> auto
            # Sorry, Japanese Only.
...
def write(outputdir, caption_list):
    for caption_id, caption in caption_list:
        with open(
            "{}/{}.ttml".format(outputdir, caption_id), "wt", encoding="utf-8"
        ) as f:
            f.write(caption)
            f.flush()
...
if __name__ == "__main__":
    ...
    channel_id = "UCD-miitqNY3nyukJ4Fnf4_A"  # 取得するチャンネルID
    num = 36
