...
class Formatter(object):
    ...
    def check_page_num(self, paper_type):
        ...
        if marker > (page_threshold + 1, 1):
            page, line = marker
            self.logs[Error.PAGELIMIT] = [
                f"Paper exceeds the page limit "
    ...
    def check_references(self):
        ...
        for i, page in enumerate(self.pdf.pages):
            ...
            if found_references:
                ...
                urls = set(
                    urls
                )  # When link text spans more than one line, it returns the same url multiple times
                for url in urls:
                    if "doi.org" in url:
                        doi_url_count += 1
        ...
        if doi_url_count < 3:
            self.logs[Warn.BIB] += [
                f"Bibliography should use ACL Anthology DOIs whenever possible. Only {doi_url_count} references do."
            ]

        if (
            arxiv_url_count > 0.2 * all_url_count
        ):  # only 20% of the links are allowed to be arXiv links
            self.logs[Warn.BIB] += [
                f"It appears you are using arXiv links more than you should ({arxiv_url_count}/{all_url_count}). Consider using ACL Anthology DOIs instead."
            ]

        if all_url_count < 5:
            self.logs[Warn.BIB] += [
                f"It appears most of the references are not using paper links. Only {all_url_count} links found."
            ]

        if arxiv_word_count > 10:
            self.logs[Warn.BIB] += [
                f"It appears you are using arXiv references more than you should ({arxiv_word_count} found). Consider using ACL Anthology references instead."
            ]
